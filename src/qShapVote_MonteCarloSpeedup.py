...
if debug:
    ...
    tempSPsiCircuit.h(
        [player for player in playerReg if player != playerReg[targetPlayer]]
    )
...
for i in range(amplitudeEstBits):
    QGatePowers.append(QGate.repeat(2**i))
...
ampCircuit = QuantumCircuit(
    len(set(auxReg + playerReg + countReg + utilReg + amplitudeReg))
)
...
output = np.sin((np.pi / 2**amplitudeEstBits) * np.arange(2**amplitudeEstBits)) ** 2
print(output)
