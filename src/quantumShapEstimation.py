...
class QuantumShapleyWrapper:
    ...
    @staticmethod
    def getShapleyInitGate(
        betaApproxBits: int, numFactors: int, target: int, targetOn: bool = True
    ) -> Gate:
    ...
    def initShapley(
        ...
        circuit.append(
            factorInitGate, regDict[Registers.aux] + regDict[Registers.factors]
        )
    ...
    def approxShap(
        self,
        targetFactor: int,
        numMeasurements: int = 2**12,
        betaApproxBits: Optional[int] = None,
        rangeMin: float = -128,
        rangeMax: float = 127,
        directProb: bool = False,
    ) -> float:
        if betaApproxBits is None:
            betaApproxBits = int(np.ceil(np.log2(len(self.__factorIndices))))
        ...
        for toggle in [self.ON, self.OFF]:
            ...
            # Simulate circuit
            result = sim.run(
                compiled_circuit, shots=numMeasurements  # if not directProb else 1
            ).result()
        ...
        if directProb:
            ...
            shap = (
                (rangeMax - rangeMin)
                * (counts[self.ON] - counts[self.OFF])
                / numMeasurements
            )
    ...
    def constructAGate(
        self, auxReg: list[int], factorReg: list[int], targetFactor: int, targetOn: bool
    ) -> Gate:
        # Defining the A Gate
    ...
    def constructVGate(self, reg: dict[Registers, list[int]]) -> Gate:
    ...
    def constructS0Gate(self, reg: dict[Registers, list[int]]) -> Gate:
        S0Circuit = QuantumCircuit(len(reg[Registers.aux] + reg[Registers.full]))
        mczGate = ZGate().control(
            num_ctrl_qubits=len(reg[Registers.aux] + reg[Registers.factors]), label="Z"
        )

        S0Circuit.x(reg[Registers.aux] + reg[Registers.full])
        S0Circuit.append(mczGate, reg[Registers.aux] + reg[Registers.full])
        S0Circuit.x(reg[Registers.aux] + reg[Registers.full])
    ...
    def constructQGate(
        ...
        invAGate = AGate.inverse()
        invAGate.label = "A^-1"
        invWGate = WGate.inverse()
        invWGate.label = "W^-1"
    ...
    def controlledGatePowers(gate: Gate, amplitudeEstBits: int):
        ...
        for i in tqdm(range(amplitudeEstBits)):
            gatePowers.append(gate.repeat(2**i))
            gatePowers[i].label = f"{gate.label}^{2**i}"
            gatePowers[i] = gatePowers[i].control()

        return gatePowers

    def approxShapAmpEst(
        self,
        targetFactor: int,
        numMeasurements: int = 2**4,
        amplitudeEstBits: int = 3,
        betaApproxBits: int = None,
        rangeMin: float = -128,
        rangeMax: float = 127,
    ) -> float:
        ...
        weights = (
            np.sin((np.pi / 2**amplitudeEstBits) * np.arange(2**amplitudeEstBits)) ** 2
        )

        shapleyEstimation = np.dot(weights, np.array(outputProbs[self.ON])) - np.dot(
            weights, np.array(outputProbs[self.OFF])
        )
...
def assessCircuit(circuit: QuantumCircuit, register: list[int]):
    ...
    angles = {key: np.sin(np.angle(value) / 2) for key, value in state.items()}
    ...
    plt.bar(x, probs, align="edge", width=0.9 / (len(probs)), color=colors)
    plt.xticks(
        ticks=x + 0.9 / (2 * len(probs)),
...
def main():
    ...
    qsw = QuantumShapleyWrapper(
        gate,
        [i for i in range(numFactors)],
        [numFactors],
        [i for i in range(numFactors + 1)],
    )
